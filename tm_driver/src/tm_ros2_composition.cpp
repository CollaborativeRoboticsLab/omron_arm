...
int main(int argc, char *argv[])
{
  ...
  // Create node first to use ROS2 parameters
  ...
  bool isSetNoLogPrint = node->get_parameter("no_log_print").as_bool();
  if (isSetNoLogPrint)
  {
  ...
  if (use_simulation)
  {
    RCLCPP_WARN(node->get_logger(), "Parameter 'tm_use_simulation' is set to true, exiting.");
    rclcpp::shutdown();
    return 1;
  }
  ...
  auto tm_svr = std::make_shared<TmSvrRos2>(node, iface, use_simulation);
  auto tm_sct = std::make_shared<TmSctRos2>(node, iface, use_simulation);
  rclcpp::spin(node);
